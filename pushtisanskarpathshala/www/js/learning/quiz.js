...
class Quiz {
	...
	get_quiz() {
		frappe.call('pushtisanskarpathshala.www.utils.custom_utils.get_quiz', {
			quiz_name: this.name,
			course: this.course
		}).then(res => {
...
//		}
	...
	view_ans(){
	    alert('before')
		frappe.call('pushtisanskarpathshala.www.utils.custom_utils.evaluate_quiz_correct', {
			quiz_name: this.name,
	...
	submit() {
		...
		this.disable()
		frappe.call('pushtisanskarpathshala.www.utils.custom_utils.evaluate_quiz', {
			quiz_name: this.name,
			quiz_response: this.get_selected(),
			course: this.course,
			program: this.program,
